...
import { ArrowLeft, Package, Star, Eye, Edit } from 'lucide-react'
      ...
      <CardContent className="p-4">
        ...
        <div className="space-y-2">
          ...
          <div className="flex items-center justify-between text-sm">
            ...
            <span className="text-muted-foreground">
              {t('Product.Sold')}: {product.numSales || 0}
            </span>
          </div>
...
async function BrandProducts({ brand, page, sort, t }: { 
  brand: string, 
  page: number, 
  sort: string,
  t: any
}) {
  const { success, products, pagination } = await getProductsByBrand({
    ...
    <div className="space-y-6">
      ...
      {pagination && pagination.pages > 1 && (
        <div className="flex justify-center items-center gap-2 mt-8">
          {page > 1 && (
            <Link 
              href={`/admin/brands&tags/brand/${brand}?page=${page - 1}&sort=${sort}`}
              scroll={false}
            >
...
export default async function BrandDetailPage({ params, searchParams }: Props) {
  const t = await getTranslations('Admin.BrandProducts')
